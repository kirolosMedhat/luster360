...
import { ArrowLeft, Download, Share2, QrCode, Play, Pause, Sparkles, Check } from 'lucide-react';
...
export default function CustomerVideoPage({ params }: VideoPageProps) {
  ...
  const currentUrl = typeof window !== 'undefined' ? window.location.href : `https://events.luster-photobooth.com/v/${shortCode}`;
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          ...
          text: 'Check out my 360 photobooth video from Ahmed & Mariam Wedding!',
          url: currentUrl,
        });
      } catch (err) {
        // Fallback to copy
  ...
  return (
    <div className="min-h-screen flex flex-col bg-luster-bg text-luster-text">
      ...
      {/* 2. Main Content Container (Mobile-first centered column) */}
      <main className="flex-1 max-w-md w-full mx-auto px-5 py-6 flex flex-col items-center">
        {/* Event Context */}
        <div className="text-center mb-5">
          ...
          <p className="text-xs text-luster-textMuted font-mono mt-0.5">
            Spin Code: #{shortCode} • 10 Sep 2026
          </p>
        </div>

        {/* 3. Cinematic Video Player Card */}
